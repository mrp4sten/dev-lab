...
- `docs/` hosts knowledge base material (`docs/cheatsheets/`, `docs/learning/`, `docs/workflows/`); keep additions concise and cross-link when possible.
- `tools/` holds versioned utilities. `tools/sonarqube/` contains the Docker Compose stack for local code quality runs; `tools/llms-prompt-examples/` stores prompt reference material.
